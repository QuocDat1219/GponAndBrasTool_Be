...
def create_New_User(user: User):
    try:
        print(conn.demo.user.find_one({"email": user.email}))
        if conn.demo.user.find_one({"email": user.email}):
            return HTTPException(status_code=400, detail={"msg": "Email đã được sử dụng"})
        
        # Mã hóa password
        hashed_password = hash_password(user.password)
        user.password = hashed_password
        
        created_user = conn.demo.user.insert_one(dict(user))
        if created_user:
            return serializeDict(conn.demo.user.find_one({"_id": created_user.inserted_id}))
...
def login_user(user: LoginModel):
    try:
        ...
        db_user = conn.demo.user.find_one({"email": user.email})
        if not db_user:
        ...
        # Tạo JWT token
        token = signJWT(str(db_user["_id"]), db_user["fullname"], db_user["role"], db_user["email"])
        return token
    except Exception as e:
...
async def change_password(change_password_model: ChangePasswordModel, token: str = Depends(jwtBearer())):
    ...
    db_user = conn.demo.user.find_one({"_id": ObjectId(user_id)})
    if not db_user:
        raise HTTPException(status_code=404, detail="Không tìm thấy tài khoản")
    ...
    conn.demo.user.update_one({"_id": ObjectId(user_id)}, {"$set": {"password": new_hashed_password}})
